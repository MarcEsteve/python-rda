# This script demonstrates how to create an agent that uses multiple tools to perform different tasks.
# The agent can convert currency, fetch the weather, get news headlines, tell jokes, provide random facts, and search Wikipedia.
# The tools are implemented as functions that make HTTP requests to various APIs.
# The agent uses the CodeAgent class, which allows the agent to call the tools directly.
# The agent can be run with different queries to demonstrate the functionality of each tool.
